...
def topomad_thres_select(score, start=85, end=99):
    search_range = [x for x in list(score) if (x >= np.percentile(score, start)) and (x<=np.percentile(score, end))]
    search_step = len(search_range)
    min_s = min(score)
    m = 0
    m_t = np.percentile(score, end)
    sorted_samples = sorted(search_range)
    for i in range(search_step-1):
        threshold = sorted_samples[i]
        target = (sorted_samples[i+1]-sorted_samples[i])/(sorted_samples[i+1]+sorted_samples[i]-2*min_s)
        if target > m:
            m_t = threshold
            m = target
    return m_t
...
def telemanom_find_epsilon(e_s, error_buffer=100, sd_lim=12.0):
    """
