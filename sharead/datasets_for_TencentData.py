...
class MonitorEntityDataset_TencentData(Dataset):
    def __init__(self, datafiles, sequence_length, z_dim, period=1440, len_test=7200, use_sfa=True, no_longterm=False, largest_bin=8, seed=0, is_iterate=True, gpu=None):
        ...
        for i in range(len(datafiles)):
            mac_id = datafiles[i].split('/')[-1].split('.')[0]
            
            example = pd.read_csv(datafiles[i], index_col=0)
            example.interpolate(inplace=True)
            example.bfill(inplace=True)
            example.fillna(0, inplace=True)
            
            for day_i in range(7):
                df = example.values[day_i*period:(day_i+1)*period]
    ...
    def feedback(self, feedback_labels: dict=None, align_preprocess: dict=None):
        self.is_feedback = True
        if feedback_labels is not None:
            self.feedback_index = []
            self.feedback_index_dict = {}
            self.align_preprocess = {}
            for mac_id in self.test_index.keys():
                if align_preprocess is not None:
                    if '_TRAINSET' not in mac_id:
                        self.align_preprocess[self.test_index[mac_id][0][0]] = torch.from_numpy(align_preprocess[mac_id]).float().to(self.device)
                else:
                    self.align_preprocess[self.test_index[mac_id][0][0]] = torch.from_numpy(np.array([0,1])).float().to(self.device)
            for mac_id in feedback_labels.keys():
                self.feedback_index.extend([(self.test_index[mac_id][loc+1], feedback_labels[mac_id][loc]) for loc in range(len(feedback_labels[mac_id]))])
                self.feedback_index_dict[mac_id] = (self.test_index[mac_id][1], feedback_labels[mac_id])
                

    def retrieve_feedback_and_corresponding_data(self, mac_id):
        feedback_data_loc, feedback_labels = self.feedback_index_dict[mac_id]
        loc_set, loc_seq = feedback_data_loc
        return self.tensors[loc_set][loc_seq-self.sequence_length+1:loc_seq+len(feedback_labels)].squeeze().to('cpu').numpy(), feedback_labels
    
    def retrieve_test_data_with_no_feedback(self, mac_id):
        feedback_data_loc, feedback_labels = self.feedback_index_dict[mac_id]
        loc_set, loc_seq = feedback_data_loc
        return self.tensors[loc_set][loc_seq-self.sequence_length+1+len(feedback_labels):].squeeze().to('cpu').numpy()
                
    def __len__(self):
        if self.is_feedback:
            return len(self.feedback_index)
        if self.is_train:
    ...
    def __getitem__(self, i):
        ...
        '''
        if self.is_feedback:
            feedback_data_loc, feedback_label = self.feedback_index[i]
            loc_set, loc_seq = feedback_data_loc
            return self.tensors[loc_set][loc_seq-self.sequence_length+1:loc_seq+1].transpose(0,2).contiguous(), self.align_preprocess[loc_set], feedback_label, loc_set
        if self.use_sfa:
            if self.is_train:
                loc_set, loc_seq= self.train_index[i]
                return self.tensors[loc_set][loc_seq-self.sequence_length+1:loc_seq+1].transpose(0,2).contiguous(), loc_set#.repeat(3, 1, 1)
            else:
